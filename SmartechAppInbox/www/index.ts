...
const kPluginName = 'SmartechAppInboxCordova';

export interface Category {
    categoryName: string;
}

export class SmartechAppInboxPlugin {
    ...
    getAppInboxMessagesWithCategory(categories: Category[], handlerSuccess: (result: object[]) => void, handlerError: (error: Error) => void): void {
        try {
            window.cordova.exec((result: string) => handlerSuccess(JSON.parse(result)), handlerError, kPluginName, "getAppInboxMessagesWithCategory", [categories]);
        } catch (error) {
...
const SmartechAppInboxCordova = new SmartechAppInboxPlugin();
